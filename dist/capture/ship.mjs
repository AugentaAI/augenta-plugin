...
async function postExperiences(url, token, experiences, connectorId, authMode = "api-key") {
  ...
  for (let i = 0;i < maxBatches; i++) {
    ...
    try {
      for (const body of packBodies(experiences)) {
        const res = await postExperiences(opts.url, opts.token, body, opts.connectorId, opts.authMode);
